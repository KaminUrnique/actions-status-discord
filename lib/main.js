...
function getPayload(inputs) {
    ...
    const { eventName, sha, ref, workflow, actor, payload, platform, downloadTitle, downloadURL } = ctx;
    ...
    const blankField = { name: '\b', value: '\b', inline: true };
    ...
    if (!inputs.nocontext) {
        embed.fields = [
            ...
            {
                name: 'Download',
                value: `[${inputs.downloadTitle}](${inputs.downloadURL})`,
                inline: true
            }
        ];
    }
